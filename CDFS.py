...
import math
'''
    Community Detection Based on the fish streaming
'''
class non_overlap_cdme:
    ...
    def store_graphlist(self, fname):
        ...
        self.st = {}  # storge the AA
        #compute the AA
        for node in self.G.nodes():
            Nv=sorted(self.G.neighbors(node))
            for u in Nv:
                Nu=self.G.neighbors(u)
                keys = str(node) + '_' + str(u)
                self.st.setdefault(keys,self.AA(set(Nv), set(Nu)))
        print('AAindex,done') 
